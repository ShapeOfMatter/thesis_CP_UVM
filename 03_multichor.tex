...
\begin{figure*}[tbhp]
  \begin{mdframed}
    ...
    \caption{
        ...
        a choreography involving only the servers which is called via \inlinecode{conclave} on line~24.
        ...
        The primary server broadcasts the request (line~6); the backups will update their state and report their health
        only for a \inlinecode{Put} request.
        On line 8 the backups call the local IO function \inlinecode{handlePut} in \inlinecode{parallel} using their individual state references;
        ...
        \inlinecode{gather} (line 9) communicates all the \inlinecode{oks} to the primary server
        where they're stored as a \inlinecode{Quire backups Response}.
        If all the backups are ok, then the primary server also handles the request (line 12).
    }
...
\begin{figure*}
  \begin{mdframed}
    \inputminted[xleftmargin=10pt,linenos,fontsize=\scriptsize,firstnumber=1,firstline=1,lastline=14]{Haskell}{figures/gmw-helpers.hs.txt}
...
\begin{figure*}
  \begin{mdframed}
    \inputminted[xleftmargin=10pt,linenos,fontsize=\scriptsize,firstnumber=1,firstline=19,lastline=65]{Haskell}{figures/gmw-helpers.hs.txt}
...
The \inlinecode{fAnd} choreography (\Cref{fig:gmw-helpers-multichor-example-b} lines~1--25)
...
On line~9, the parties perform a \inlinecode{fanOut} to begin the pairwise computation;
the \inlinecode{fanIn} on line~11 completes the pairing,
and uses \inlinecode{conclaveTo} (line 18) to embed pairwise OTs (via \inlinecode{ot2}) in the larger set of parties.
