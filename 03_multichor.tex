\chapter{The MultiChor Library for Haskell}
...
\begin{figure*}[tbhp]
  ...
  \end{tabular}
    ...
    %\Description{Fourteen lines of Haskell code using the MultiChor library.}
  \end{mdframed}
\end{figure*}
...
We have already shown a few programs written in \MultiChor, however these have been bespoke examples crafted for the purpose of demonstrating \MultiChor.
\Cref{sec:mpc} describes our implementation of the Goldreich-Micali-Widgerson protocol for secure multiparty computation.
...
In \Cref{sec:usability} we discuss other people's experience writing programs in \MultiChor.
No quantitative data on this subject exists yet,
but we have received sufficient (negative) feedback to motivate some future work discussed in \Cref{sec:conclusion}.
...
\paragraph{Computing secret-shared AND gates}
...
rather than skipping the $i=j$ cases they simply force all $a_{ii}$ and $b_{ii}$ to be $0$ (\inlinecode{False}).
The implementing choreography is \inlinecode{fAnd} (\Cref{fig:gmw-helpers-multichor-example-b} lines~1--27);
it takes \inlinecode{Faceted} values holding the parties' shares of the input values,
...
and uses \inlinecode{conclaveTo} (line~20) to embed pairwise OTs (via \inlinecode{ot2}, defined on line~33) in the larger set of parties.
The variable names used in \Cref{fig:gmw-helpers-multichor-example-b} attempt to reflect the homogeneous perspective of
...
as described in \Cref{sec:membership}.
...
\paragraph{The \inlinecode{CanSend} type class}
...
contributes nothing to the management of \inlinecode{import} statements,
and is of dubious benefit even to the users who know the system best: ourselves.
We will discuss practical implications of the above points in \Cref{sec:conclusion}.
