...
\begin{figure}[tbhp]\caption{A Simple Concurrent Protocol: a key-value store with a backup server}
  ...
  \end{minipage} \\\\
  \begin{minipage}{12.5cm}
  The function to be called by the client process.
  ...
  The function to be called by the primary server.
	  They pass in a reverence to their mutable state, and receive a message of type \inlinecode{Request} from the client.
  ...
  The function to be called by the backup server.
	  They pass in a reverence to their mutable state, and receive a \inlinecode{Put} message from the primary server.
