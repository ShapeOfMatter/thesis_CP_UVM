...
For example, EPP of the choreography in \Cref{fig:kvsenclave}\emph{(b)} to each of the three participants
...
\Cref{fig:kvsenclave}\emph{(a)} implements HasChor's KoC strategy: the branch guard is broadcast to everyone.
HasChor's authors knew this to be an inefficient (but expedient) solution.
As we show in \Cref{fig:kvsenclave}\emph{(b)}, \MultiChor can do better.
...
\begin{figure}[tbhp]
  ...
  \end{minipage} \\\\
  \begin{minipage}{0.95\linewidth}
	  This pseudo-code choreography implements the protocol from \Cref{fig:kvsenclave}\emph{(b)}
...
As a direct port of HasChor, UniChorn does not support enclaves, MLVs, or census polymorphism.
...
Klor, on the other hand, is effectively an enclaves-\&-MLVs system, but their API differs from the implementations we present here,
...
that is somewhat similar in spirit to our enclaves.
However, applying the enclave concept to choreographic programming, and to the choreographic knowledge-of-choice problem in particular,
